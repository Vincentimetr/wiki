#include "..\..\script_macros.hpp" //Pour le life décommentez

/* 
TRIGGER:
Size minimum: 10*10*4 (si trop petit les joueurs trop rapides ne seront pas détectés)
type: None
Activation: Anyplayer
Condition: this
OnActivation: if ((vehicle player isKindOf "car") && ((player distance ThisTrigger)<(((triggerArea ThisTrigger)select 0 ) max ((triggerArea ThisTrigger)select 1 )*1.5)))then {player setVariable["Circuit",1,true];ThisTrigger call compile preprocessFileLineNumbers 'core\scripts\Circuit\Verification.sqf';}; 
*/

	if ((player GetVariable["Circuit",0])==1) then {(player SetVariable["nb_trigger",4,true])};
	if ((player GetVariable["Circuit",0])==2) then {(player SetVariable["nb_trigger",6,true])};
	if ((player GetVariable["Circuit",0])==3) then {(player SetVariable["nb_trigger",6,true])};
	if ((player GetVariable["nb_trigger",0])==0) exitwith{};
